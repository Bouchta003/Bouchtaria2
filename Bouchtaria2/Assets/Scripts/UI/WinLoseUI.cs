...
public class WinLoseUI : MonoBehaviour
{
    ...
    public void ShowWin()
    {
        if (GameRunContext.IsDungeonRun) DungeonManager.ApplyCombatResult(playerWon: true);
        Setup("VICTORY", Color.green, "Enemy Core Destroyed, you earned 20 Dungeon Coins");
    }

    public void ShowLose()
    {
        if (GameRunContext.IsDungeonRun) DungeonManager.ApplyCombatResult(playerWon: false);
        Setup("DEFEAT", Color.red, "Your Core Was Destroyed, dungeon run has been reset");
    }
